      ...
      body: Stack(
        children: [
          ...
          SafeArea(
            child: Column(
              children: [
                ...
                // İzleyici (elenmiş) oyuncu cevap veremez.
                Expanded(
                  child: IgnorePointer(
                    ignoring: state.isSpectator,
          ...
          if (state.isSpectator && state.status != 'finished')
            _SpectatorBanner(onLeave: () => context.go('/home')),
...
class _PlayerAvatar extends StatelessWidget {
  const _PlayerAvatar({required this.player, this.size = 36, this.frame});
...
class _SpectatorBanner extends StatelessWidget {
  const _SpectatorBanner({this.onLeave});
